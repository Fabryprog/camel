...
The Paho component supports 15 endpoint options which are listed below:
...
| autoReconnect | common | true | boolean | Client will automatically attempt to reconnect to the server if the connection is lost
...
| password | common |  | String | Password to be used for authentication against the MQTT broker
...
| userName | common |  | String | Username to be used for authentication against the MQTT broker
