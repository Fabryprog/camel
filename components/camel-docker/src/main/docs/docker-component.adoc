...
The Docker component supports 15 options which are listed below.
...
The Docker component supports 19 endpoint options which are listed below:
...
| socket | common | true | boolean | connection mode is socket is disabled the client will use tcp mode
| tlsVerify | common | false | boolean | doker with trusted CA certificate
